...
pub(in super::super) fn build_timeout_ribbon<'a>(
    ...
    let black = match snapshot.timeout {
        ...
        TimeoutSnapshot::Black(_) => make_multi_label_message_button(
    ...
    let white = match snapshot.timeout {
        ...
        TimeoutSnapshot::White(_) => make_multi_label_message_button(
            (fl!("end-timeout-line-1"), fl!("end-timeout-line-2")),
            Some(Message::EndTimeout),
        )
        .style(ButtonStyle::Yellow),
    ...
    let referee = match snapshot.timeout {
        ...
        .style(ButtonStyle::Yellow),
        TimeoutSnapshot::Ref(_) => make_multi_label_message_button(
            (fl!("end-timeout-line-1"), fl!("end-timeout-line-2")),
            Some(Message::EndTimeout),
        )
        .style(ButtonStyle::Yellow),
    ...
    let penalty = match snapshot.timeout {
        ...
        TimeoutSnapshot::PenaltyShot(_) => make_multi_label_message_button(
            (fl!("end-timeout-line-1"), fl!("end-timeout-line-2")),
            Some(Message::EndTimeout),
        )
        .style(ButtonStyle::Yellow),
...
pub(super) fn make_game_time_button<'a>(
    ...
    let timeout_info = match snapshot.timeout {
        ...
        TimeoutSnapshot::Ref(_) => Some((fl!("ref-timeout-short"), TextStyle::Yellow)),
        TimeoutSnapshot::PenaltyShot(_) => Some((fl!("penalty-shot-short"), TextStyle::Red)),
...
pub(super) fn limit_team_name_len(name: &String, len_limit: usize) -> String {
...
pub(super) fn config_string(
    ...
    result += &fl!(
        ...
        team_timeouts = config.team_timeouts_per_half,
        stop_clock_last_2 = "Unknown"
    );
    result += "\n";
